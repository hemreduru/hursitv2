<div class="max-w-4xl mx-auto space-y-8">
    <section>
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ __('messages.admin_profile_settings') }}</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">{{ __('messages.admin_manage_profile') }}</p>
            </div>
            <a href="{{ route('admin.profile.create') }}" class="flex items-center justify-center gap-2 bg-primary hover:bg-blue-600 text-white px-4 py-2 rounded-lg font-medium shadow-lg shadow-primary/20 transition-all active:scale-95 text-sm">
                <span class="material-symbols-outlined text-lg">add</span>
                {{ __('messages.admin_new_profile') }}
            </a>
        </div>
