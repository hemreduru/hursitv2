<footer class="border-t border-border-light dark:border-border-dark bg-background-light dark:bg-background-dark py-12">
    <div class="w-full max-w-[960px] mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row items-center justify-between gap-6">
        <div class="flex flex-col gap-2 text-center md:text-left">
            <h3 class="font-bold text-lg">{{ $profile->name ?? 'Hurşit Emre Duru' }}</h3>
            <p class="text-sm text-slate-500 dark:text-slate-400">{{ $profile->title ?? 'Senior Full-Stack Engineer' }}</p>
        </div>
        <div class="flex gap-4">
            @if(isset($profile) && $profile->social_links)
                @foreach($profile->social_links as $platform => $url)
                    <a class="p-2 text-slate-400 hover:text-primary transition-colors" href="{{ $url }}" target="_blank" rel="noopener noreferrer">
